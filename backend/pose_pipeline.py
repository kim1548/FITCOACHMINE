...
def compute_category_scores(event_groups, significant, result, penalty_per_type):
    ...
    # rep 카운트는 신뢰도가 낮아 점수에 반영하지 않는다 (coverage만 사용)
    scores["ROM"] = 60.0 + 40.0 * coverage
...
def build_overall_review(exercise_name, total_score, rep_count, category_scores, significant, event_groups):
    ...
    s1 = (
        f"{CATEGORY_LABELS[best_cat]}는 거의 흠잡을 데 없이 "
        f"{category_scores[best_cat]:.0f}점으로 잘 나왔어요."
    )
    ...
    if significant:
        ...
        primary_label = OVERLAY_MESSAGES.get(primary, primary)
        s3 = (
            f"다만 동작 구간에서 '{primary_label}' 패턴이 {len(primary_evs)}회, "
...
# 권장 촬영 각도에서 판정 불가한 오류는 무시한다.
# 데드리프트는 '측면' 촬영이라 그립 너비(좌우 손 간격)를 볼 수 없으므로 grip 오류를 끈다.
SUPPRESSED_ERRORS = {
    "데드리프트": {"arms_narrow", "arms_spread"},
}

# 운동-종류 검증: 자세를 (다리 방향 + 상체 방향)으로 3분류한다.
#   upright = 서서 상체 수직 (스쿼트류)
#   hinge   = 서서 상체 숙임 (데드리프트류 — 힙 힌지라 상체가 수평으로 보임)
#   lying   = 다리가 수평 = 누움 (벤치류)
# 자세가 애매한 운동(인클라인 벤치·런지·머신플라이 등)은 검증 제외(항상 통과).
POSTURE = {
    "스쿼트": "upright",
    "오버헤드 프레스": "upright",
    "사이드 레터럴 레이즈": "upright",
...
def _avg(landmarks, i, j, k):
    return (landmarks[i][k] + landmarks[j][k]) / 2


def _posture(landmarks, bbox):
    """다리(골반→발목)·상체(어깨→골반) 방향으로 자세 분류: upright/hinge/lying/None.
    bbox로 픽셀 종횡비를 복원해 각도를 정확히 판단."""
    try:
        x1, y1, x2, y2 = bbox
        cw = max(x2 - x1, 1)
        ch = max(y2 - y1, 1)
        sh_x, sh_y = _avg(landmarks, 11, 12, 0), _avg(landmarks, 11, 12, 1)
        hp_x, hp_y = _avg(landmarks, 23, 24, 0), _avg(landmarks, 23, 24, 1)
        an_x, an_y = _avg(landmarks, 27, 28, 0), _avg(landmarks, 27, 28, 1)
        sv = min(landmarks[11][3], landmarks[12][3])
        hv = min(landmarks[23][3], landmarks[24][3])
        av = min(landmarks[27][3], landmarks[28][3])
    except (IndexError, TypeError):
        return None
    if sv < 0.3 or hv < 0.3:
        return None
    # 다리가 수평이면 '누운' 자세 (발목이 충분히 보일 때만)
    if av >= 0.3:
        ldx = abs((hp_x - an_x) * cw)
        ldy = abs((hp_y - an_y) * ch)
        if ldx > ldy * 1.3:
            return "lying"
    # 상체 방향으로 upright(수직) / hinge(숙임) 구분
    tdx = abs((sh_x - hp_x) * cw)
    tdy = abs((sh_y - hp_y) * ch)
    if tdy > tdx * 1.3:
        return "upright"
    if tdx > tdy * 1.3:
        return "hinge"
...
def _majority_contradicts(landmarks_by_frame, classify, expected):
    """프레임별 분류가 기대값과 강하게(분명한 프레임의 60% 초과) 모순되는지."""
    clear = contradict = 0
    for fr in landmarks_by_frame.values():
        v = classify(fr)
        if v is not None:
            clear += 1
            if v != expected:
                contradict += 1
    return clear >= 8 and (contradict / clear) > 0.6


def _verify_exercise_match(exercise_name, landmarks_by_frame):
    """업로드 영상의 자세가 선택한 운동과 강하게 모순되면 False(불일치)."""
    expected = POSTURE.get(exercise_name)
    if expected is None:
        return True
    if _majority_contradicts(
        landmarks_by_frame,
        lambda fr: _posture(fr.get("landmarks"), fr.get("crop_bbox")),
        expected,
    ):
        return False
    return True
...
def _knee_angle(landmarks):
    """엉덩이-무릎-발목 각도(도). 더 잘 보이는 다리 사용. 설 때 ~180, 앉을수록 작아짐."""
    L = (23, 25, 27)
    R = (24, 26, 28)
    try:
        def vis(idx3):
            return min(landmarks[i][3] for i in idx3)
        leg = L if vis(L) >= vis(R) else R
        if vis(leg) < 0.3:
            return None
        hip, knee, ankle = landmarks[leg[0]], landmarks[leg[1]], landmarks[leg[2]]
        return _calc_angle((hip[0], hip[1]), (knee[0], knee[1]), (ankle[0], ankle[1]))
    except (IndexError, TypeError):
        return None


# 운동을 '실제로 수행 중'인 프레임만 채점하기 위한 게이트.
# 모델의 down/up이 서 있는 셋업까지 'down'이라 하므로, 기하학적 신호로 직접 판단한다.
SQUAT_ACTIVE_KNEE_ANGLE = 150.0  # 무릎이 이보다 굽혀졌을 때만 '스쿼트 중'으로 본다


def _is_active_frame(exercise_name, landmarks, bbox):
    """해당 운동을 실제 수행 중인 프레임이면 True, 셋업/락아웃이면 False, 판단 불가/미적용이면 None."""
    if exercise_name == "스쿼트":
        ka = _knee_angle(landmarks)
        if ka is None:
            return None  # 무릎이 안 보이면 판단 보류 → 호출부에서 모델 단계로 폴백
        return ka < SQUAT_ACTIVE_KNEE_ANGLE
    if exercise_name == "데드리프트":
        # 힙 힌지로 상체가 숙여진(hinge) 구간만 채점. 서서 락아웃(upright)은 제외.
        p = _posture(landmarks, bbox)
        if p is None:
            return None
        return p == "hinge"
    if exercise_name == "벤치프레스":
        # 누운(lying) 구간만 채점. 세트 사이 일어나 앉은 자세는 제외.
        p = _posture(landmarks, bbox)
        if p is None:
            return None
        return p == "lying"
    return None  # 그 외 운동은 게이트 미적용
...
def analyze_video(video_path, exercise_name, frame_skip, yolo_conf, progress_cb=None):
    ...
    suppressed = SUPPRESSED_ERRORS.get(exercise_name, set())
    ...
    # 속도: YOLO는 무거우므로 매 분석프레임마다 돌리지 않고 일정 간격마다만 재검출,
    # 그 사이에는 직전 bbox 재사용(세트 중 사람/카메라 위치는 거의 고정).
    cached_bbox = None
    rf_frame_count = 0
    YOLO_EVERY = 5
    ...
    try:
        while True:
            ...
            if ptype == "rf_yolo":
                # 일정 간격마다만 YOLO 재검출, 그 외엔 직전 bbox 재사용 (속도)
                run_yolo = (cached_bbox is None) or (rf_frame_count % YOLO_EVERY == 0)
                rf_frame_count += 1
                bbox = cached_bbox
                if run_yolo:
                ...
                # '실제 운동 중' 프레임만 채점(서 있는 셋업/락아웃 제외).
                #   스쿼트: 무릎이 굽혀졌을 때만 (모델이 standing을 down이라 해도 무릎 각도로 차단)
                #   판단 불가/미적용 운동: 모델 단계(down)로 폴백
                active = _is_active_frame(exercise_name, landmarks_full, bbox)
                if active is None:
                    active = (stage == "down")
                # 권장 각도에서 판정 불가한 오류(측면 데드의 그립 너비 등)도 제외
                _rec = error_key if (error_key and active and error_key not in suppressed) else None
                _record_errors([_rec] if _rec else [], frame_idx)
    ...
    exercise_match = _verify_exercise_match(exercise_name, landmarks_by_frame)
